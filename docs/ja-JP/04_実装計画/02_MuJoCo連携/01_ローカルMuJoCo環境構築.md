...
`examples/mujoco/minimal_body/model.xml` を使用して、MuJoCo が MJCF を読み込めることを確認する。

```powershell
python - <<'PY'
import mujoco

model = mujoco.MjModel.from_xml_path('examples/mujoco/minimal_body/model.xml')
...
```powershell
python -c "import mujoco; m=mujoco.MjModel.from_xml_path('examples/mujoco/minimal_body/model.xml'); d=mujoco.MjData(m); mujoco.mj_step(m,d); print('ok', m.nbody, m.njnt, m.ngeom)"
```
...
```powershell
python -m mujoco.viewer --mjcf=examples/mujoco/minimal_body/model.xml
```
...
1. headless 動作確認用 test の追加
2. position actuator サンプルの追加
3. controller_config schema の追加
4. diagnostics schema の追加
5. SansaVRM API 連携前提の adapter skeleton 作成
